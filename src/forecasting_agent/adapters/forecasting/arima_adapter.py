...
class ARIMAAdapter(ForecastingModel):
    def __init__(self, config):
        self.config = config
        self.model = DartsARIMA(**config)
        self.fitted = False
    ...
    def forecast(self, horizon: int, frequency: str = 'D') -> dict:
        ...
        # Darts ARIMA provides prediction intervals via predict(..., num_samples=100)
    ...
    def plot_forecast(self, forecast: dict, save_path: str = None):
        ...
        plt.legend()
        plt.title('ARIMA Forecast (Darts)')
        if save_path:
